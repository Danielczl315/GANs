class LSGAN():
    ...
    def train_G(self):
        ...
        loss =  torch.mean(torch.mul(pred-Variable(torch.ones(self.batch_size)).to(device), pred-Variable(torch.ones(self.batch_size)).to(device)))
        loss.backward()
        self.g_optim.step()
    ...
    def train_D(self, real_images):
        ...
        real_loss = torch.mean(torch.mul(real_pred-Variable(torch.ones(self.batch_size)).to(device), real_pred-Variable(torch.ones(self.batch_size)).to(device)))

        #real_loss.backward()
        
        # fake, close to 0
        fake_pred = self.D(fake_samples)
        fake_loss = torch.mean(torch.mul(fake_pred, fake_pred))
        #fake_loss.backward()
